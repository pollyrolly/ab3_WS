...
class App extends React.Component {
  ...
  start_socket() {
        ...
        .then(response => {
          ...
          if (!authenticated) {
            ...
            axios.get(endPoint + '/api/messages')
                .then(response => {
                    this.setState({ newmessages: response.data.newmessages });
    ...
    socket.on('message', msg => {
      ...
      this.setState(prevState => ({ mqttmessages: prevState.mqttmessages.concat(`\n${dateString} - ${JSON.stringify(msg["payload"])}`) }))
    });
        ...
        <header className="App-header">
          ...
          {authenticated &&
            <div>
              ...
              <br />
              <div className="mb-3">
                <label htmlFor="incoming_messages" className="form-label">Incoming messages from AWS IoT Core (on connect) </label>
                <div id="incoming_messages_help" className="form-text">Subscribed to the topic AWS_IOT_THING_NAME/publish</div>
                <textarea disabled rows="5" className="form-control" id="incomingMessages" name="incomingMessages" 
                                            aria-describedby="incoming_messages_help" value={this.state.incomingMessages} onChange={this.handleInputChange} />
              </div>
              <br />
              <div className="mb-3">
                <label htmlFor="mqttmessages" className="form-label">Incoming messages from AWS IoT Core (api messages soket connect)</label>
                <div id="mqttmessages_help" className="form-text">Subscribed to the topic AWS_IOT_THING_NAME/subscribe</div>
                <textarea disabled rows="5" className="form-control" 
                                         id="mqttmessages" 
                                         name="mqttmessages" 
                                         aria-describedby="mqttmessages_help" value={this.state.mqttmessages} onChange={this.handleInputChange} />
              </div>
              ...
              <div className="mb-3">
                ...
                <input id="publishString" name="publishString" placeholder="String to publish" 
                                          type="text" 
                                          className="form-control" value={this.state.publishString} onChange={this.handleInputChange} />
              </div>
